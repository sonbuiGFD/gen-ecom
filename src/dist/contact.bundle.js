...
__webpack_require__(/*! ./contact.scss */ "./src/contact/contact.scss");

__webpack_require__(/*! ./map */ "./src/contact/map.js");
...
(function ($, window) {
  ...
  var zoomInit = 6;
  var zoom = 12;
  var iconUrl = 'https://www.aviva.com.vn/Data/Sites/1/media/default/map-icon.png';
  ...
  Plugin.prototype = {
    init: function init() {
      ...
      // display map
      this.mapView = new google.maps.Map(document.getElementById('map_canvas'), mapOptions);
      this.getStores();
    ...
    renderMapStore: function renderMapStore() {
      ...
      $.each(that.stores, function (i, store) {
        ...
        marker = new google.maps.Marker({
          position: new google.maps.LatLng(parseFloat(data.lat[0]), parseFloat(data.lng[0])),
          title: data.name[0],
          icon: iconUrl,
          map: that.mapView
        });
        ...
        google.maps.event.addListener(marker, 'click', function (marker) {
          return function () {
            infoWindow.setContent('<h3 class="color-main" style=\'text-align: center;margin: 0;\'>\n                  ' + data.name[0] + '\n                  </h3>\n                  <p>\u0111\u1ECBa ch\u1EC9: ' + data.address[0] + '</p>');
    ...
    renderProvincesOptions: function renderProvincesOptions() {
      var that = this;
      var options = '<option value="">Chọn Tỉnh / Thành Phố</option>';
      var uniqueProvince = uniqueBy(that.stores, function (store) {
        return store.metadata.province_code[0];
      });
      uniqueProvince.forEach(function (item) {
        options += '<option value="' + item.metadata.province_code[0] + '">' + item.metadata.province[0] + '</option>';
      });
      this.provincesOptions.html(options);
    },

    renderDistrictsOptions: function renderDistrictsOptions() {
      var that = this;
      ...
      if (that.selectedProvince && that.selectedProvince !== '') {
        ...
        var uniqueDistrict = uniqueBy(filteredProvince, function (store) {
          return store.metadata.district_code[0];
        });

        uniqueDistrict.forEach(function (item) {
          options += '<option value="' + item.metadata.district_code[0] + '">' + item.metadata.district[0] + '</option>';
        });
      }
      this.districtsOptions.html(options);
    },

    renderListStore: function renderListStore() {
      var that = this;
      ...
      list.forEach(function (store) {
        var data = store.metadata;
        res += '\n        <li class="list_store_item" data-id="' + store.id + '">\n            <h4>' + data.name[0] + '</h4>\n          <div>\n            <strong><img src="./assets/img/contact_map_location.svg"/></strong> <span>' + data.address[0] + '</span>\n          </div>\n          <div>\n            <strong><img src="./assets/img/contact_map_phone.svg"/></strong> <span>' + data.phone[0] + '</span>\n          </div>\n          <div>\n            <strong><img src="./assets/img/contact_map_email.svg"/></strong> <span>' + data.email[0] + '</span>\n          </div>\n         </li>\n        ';
      });
